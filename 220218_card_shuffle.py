def card_shuffle(card_ls):
    ...
    shuffled_ls.append(half1_ls[-1]) 
    shuffled_ls.append(half2_ls[-1]) if even else None # 如果牌是奇數，那第2份牌就沒有最後一張
    return shuffled_ls

file_from = './in.txt'
with open(file_from, "r") as f:
    for line in f: # 等同不停的去readline()，直到沒有EOF
        if len(line.strip()) != 0: # 忽略空行
            card_ls = [int(i) for i in line.strip().split()]
            shuffled_ls = card_shuffle(card_ls)
            for j in shuffled_ls:
                print(j, end=' ')
            print()
